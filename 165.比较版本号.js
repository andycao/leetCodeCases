...
var compareVersion = function(version1, version2) {
  const list1 = version1.split('.').map(e => Number(e));
  const list2 = version2.split('.').map(e => Number(e));
  
  // 递归比较位数
  const compareIndex = (index) => {
    if(index > list1.length - 1 && index > list2.length - 1) {
      return 0;
    }
    const d1 = list1[index] || 0;
    const d2 = list2[index] || 0;
    if(d1 === d2) {
      return compareIndex(index + 1);
    } else if(d1 > d2) {
      return 1;
    } else {
      return -1;
